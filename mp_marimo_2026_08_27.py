...
def _(checkbox, mo, multiselect, radio):
    mo.vstack([
        mo.md('<h1>Interact with Marimo widgets</h1>'),
        mo.hstack([
            radio,
        ...
        mo.hstack([
            checkbox,
            mo.left(mo.Html('Have read them' if checkbox.value else 'Have NOT read them'))
        ],
        widths='equal'),
    ])
    return
...
def _(df_polars, mo):
    mo.vstack([
        mo.md('Single digit "Times Table"'),
...
def _(df_cal, mo):
    mo.vstack([
        mo.md("Calfornia Counties - population statistics"),
        df_cal
    ])
...
def _(county_info, demo, demo_view, description, fig1_ui, fig2_ui, mo):
    layout = mo.vstack([
        mo.Html('<br>'),
        mo.left(mo.md(f' <h1>California County {demo_view}</h1>')),
        mo.Html('<br>'),
        mo.Html(description),
        county_info,
        mo.left(demo),
...
if __name__ == "__main__":
    app.run()
I’ll be talking about marimo notebooks at the next Bay Area Python Interest Group meeting. I’ll share examples and ideas from my notebook, mp_marimo_2026_08_27.py, including how marimo makes Python notebooks reactive, reproducible, and app-friendly. Hope to see you there!
