...
def _(cs, pl):
    ...
    df = (
        ...
        .with_columns(pl.col('County').str.replace(' County, California',''))
        .with_columns(pl.col('County').str.slice(1, None))  # drop the pesky leading period
        .with_columns(cs.contains('RANK').cast(pl.UInt8))
    )
    df
    return
